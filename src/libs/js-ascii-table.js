...
(function () {
	...
	JSAsciiTable.prototype.toString = JSAsciiTable.prototype.render = function () {
		var data = this.data;
		...
		for (var i = 0; i < data.length; i++) {
			...
			for (var j = 0; j < data[i].length; j++) {
				// for each column
				if (
					typeof col_length[j] == "undefined" ||
					col_length[j] < data[i][j].length
				) {
	...
	var _isNumeric = function (input) {
		return (
			...
			(typeof input === "string" && input.toLowerCase() == "null")
		);
	};

	...
	GamerNo: class GamerNo {
		...
		getUserHeaderImg(html) {
			...
			const imgReg = new RegExp(
				"background-image: url\\(//(img.gfx.no/.*\\..*)\\)"
			);
		...
		getTwitchTag(html) {
			const twitchRegEx = new RegExp(
				'<a href="https://www.gamer.no/tv/.*">(.*)</a>'
			);
		...
		// Get Epic Tag
		getEpicTag(html) {
			const thisRegEx = new RegExp(
				'<img title="Epic Games" src="https://static.gfx.no/images/gamer/epic.png" /> <span>(.*)</span>'
			);
